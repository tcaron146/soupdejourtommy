  ...
  if (item.type === "video") {
    return (
      <figure className="my-12 w-full overflow-hidden rounded-2xl">
        <video src={item.url} autoPlay muted loop playsInline className="w-full block" />
      </figure>
    );
  }
  return (
    <figure className="my-12 w-full overflow-hidden rounded-2xl">
      <img src={item.url} alt="" className="w-full block object-cover max-h-[70vh]" />
    </figure>
  );
}

function StoryContent({ content, media = [] }) {
  if (!media.length) return <div className={TEXT_CLASS}>{content}</div>;
  ...
  return (
    <article className="pt-28 max-w-2xl mx-auto px-6 pb-20">
      ...
      <header className="mb-10 pb-8 border-b border-neutral-800/60">
        ...
        <h1 className="font-bold text-white leading-tight tracking-tight text-4xl sm:text-5xl">
          {story.title}
        </h1>
        <div className="flex items-center gap-3 mt-4 flex-wrap">
          {formattedDate && <span className="text-sm text-neutral-600">{formattedDate}</span>}
